const p=document.getElementById("loadingAuth"),x=document.getElementById("mainContent"),c=document.getElementById("kvStatus"),B=document.getElementById("totalAds"),k=document.getElementById("logoutButton"),f=document.getElementById("adsList"),I=document.getElementById("createAdButton"),g=document.getElementById("adModal"),b=document.getElementById("closeModal"),L=document.getElementById("cancelButton"),E=document.getElementById("modalTitle"),v=document.getElementById("adForm"),D=document.getElementById("editDefaultButton"),h=document.getElementById("defaultAdModal"),w=document.getElementById("closeDefaultModal"),A=document.getElementById("cancelDefaultButton"),$=document.getElementById("defaultAdForm"),S=document.getElementById("validateDefaultButton"),J=document.getElementById("defaultZhTitle"),M=document.getElementById("defaultEnTitle");let i=null,m=null;async function N(){try{(await fetch("/api/admin/test-auth")).ok?(p.classList.add("hidden"),x.classList.remove("hidden"),T(),y()):window.location.href="/admin/login"}catch(n){console.error("Auth check failed:",n),window.location.href="/admin/login"}}async function T(){try{const n=await fetch("/test-kv?action=status"),e=await n.json();n.ok?(c.textContent=e.mode.includes("Mock")?"⚠️ Mock":"✓ Connected",c.className=e.mode.includes("Mock")?"text-2xl font-bold text-yellow-600 dark:text-yellow-400":"text-2xl font-bold text-green-600 dark:text-green-400"):(c.textContent="✗ Error",c.className="text-2xl font-bold text-red-600 dark:text-red-400")}catch{c.textContent="✗ Failed",c.className="text-2xl font-bold text-red-600 dark:text-red-400"}}async function y(){try{const n=await fetch("/api/admin/ads"),e=await n.json();n.ok&&e.success?(i=e.data,B.textContent=e.count.toString(),C(e.data.schedules),e.data.default&&(J.textContent=e.data.default.zh?.title||"Not configured",M.textContent=e.data.default.en?.title||"Not configured")):f.innerHTML=`
