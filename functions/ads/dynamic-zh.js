/**
 * Dynamic Advertisement Endpoint - Chinese
 *
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!activeSchedule) {
      const defaultAd = adsData.default?.zh;
      if (!defaultAd) {
        return fallbackResponse(headers);
      }
    ...
    const zhVariants = activeSchedule.variants?.zh || [];
    let selectedVariant = zhVariants.find(v => v.version === requestedVersion);

    // Fallback to version 1 if requested version not found
    if (!selectedVariant && requestedVersion !== 1) {
      selectedVariant = zhVariants.find(v => v.version === 1);
    }
...
<div class="relative my-12 not-prose font-sans">
  ...
  <div class="flex justify-end mt-2 mr-1">
    <a href="/zh/sponsorship/" target="_blank" class="
